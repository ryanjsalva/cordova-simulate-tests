 class  App {
     ...
     init():void {
        ...
        this.back.addEventListener("click", function (e: any): void {
            this.goto("back", "left");
        }.bind(this));
    ...
    historify(url:string):string {
     ...
     enter(el: HTMLElement, direction: string):void {
        ...
        // load the new view off-screen
         let x: number = document.body.offsetWidth;
         if (direction === "left") {
             x = -x;
         }
    ...
    exit (el: HTMLElement, direction:string):void {
        ...
        var x: number = -document.body.offsetWidth;
        if (direction === "left") {
            x = -x;
        }
    ...
    hydrate(el: HTMLElement):void {
        ...
        for ( var i:number=0; i<hrefs.length; i++ ) {
        ...
        for ( var i:number=0; i<scripts.length; i++ ) {
            this.require(scripts[i].getAttribute("data-script"));
        }
        let title: NodeListOf<Element> = el.querySelectorAll(".title");
        if (title.length > 0) {
...
document.addEventListener("deviceready", function (e: any): void { new App().init(); }, false);
document.addEventListener("pause", function (e: any): void { console.log("pause event"); }, false);
document.addEventListener("resume", function (e: any): void { console.log("resume event"); }, false);
document.addEventListener("searchButton", function (e: any): void { console.log("search event"); }, false);
document.addEventListener("backButton", function (e: any): void { console.log("back event"); }, false);
